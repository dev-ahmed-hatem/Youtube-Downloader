...
def seq_stream(
    ...
    split_url = parse.urlsplit(stream_object.url)
    ...
    querys['sq'] = 0
    stream_object.url = base_url + parse.urlencode(querys)

    segment_data = b''
    for chunk in stream(stream_object.url, timeout=timeout, max_retries=max_retries):
        yield chunk
        segment_data += chunk
    ...
    while seq_num <= segment_count:
        ...
        yield from stream(stream_object.url, timeout=timeout, max_retries=max_retries)
        seq_num += 1
    return
...
def stream(
    ...
    downloaded = stream_object.start_pos
    while downloaded < stream_object.filesize:
        tries = 0
        ...
        while True:
            ...
            try:
                response = _execute_request(
                    stream_object.url,
                    headers={"Range": f"bytes={stream_object.start_pos}-{stream_object.filesize}"},
                    method="GET",
                    timeout=timeout
                )
...
def accept_ranges(url):
    """Fetch Accept-Ranges of file at given URL

    :param str url: The URL to get the size of
    :returns: int: size in bytes of remote file
    """
    return head(url)["accept-ranges"]
