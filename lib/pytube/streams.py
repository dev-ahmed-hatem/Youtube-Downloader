...
class Stream:
    ...
    def __init__(
        ...
        # Implement downloading state for stream
        self.download_state = True

        # Start position if stream exists incomplete
        self.start_pos = 0
    ...
    def accept_ranges(self) -> int:
        """Accept-Ranges property of the media stream.

        :rtype: str
        :returns:
            accept-ranges header in the stream url.
        """
        try:
            self._accept_ranges = request.accept_ranges(self.url)
        except HTTPError as e:
            if e.code != 404:
                raise
    ...
    def download(
        ...
        if self.accept_ranges == "bytes" and self.exists_incomplete(file_path):
            self.start_pos = os.path.getsize(file_path)
